...
static kernel_sync_handler s_kernel_sync_handlers[NUM_EXCEPTION_EC] = {0};
...
kernel_sync_handler get_kernel_sync_handler(uint32_t ec) {
    ...
    return s_kernel_sync_handlers[ec];
}

void set_kernel_sync_handler(uint32_t ec, kernel_sync_handler handler) {

    ASSERT(ec < NUM_EXCEPTION_EC);
    ASSERT(s_kernel_sync_handlers[ec] == NULL);
    ASSERT(handler != NULL);

    s_kernel_sync_handlers[ec] = handler;
}
